...
const Header = () => {
  ...
  const {cartProducts, totalQuantity} = useSelector((state) => state.cart)
  ...
  return (
    <header className="flex flex-col lg:mx-24 mx-[10px]  justify-between items-center font-pop border-b">
      <div className="flex w-full h-[70px] justify-between items-center bg-transparent">
        <div className="text-2xl font-bold w-[200px]"><Link to="/" onClick={() => setPage("home")} >E <span className="font-normal">STORE</span></Link></div>
        ...
        <div className="flex gap-4  lg:gap-8 lg:grow items-center lg:justify-end">
          ...
          <Link to="/cart" onClick={() => setPage("cart")} className={page == "cart" ? "text-[#717fe0] relative hover:text-[#717fe0]" : "relative hover:text-[#717fe0]"} ><IoMdCart size={24} /><p className="z-1 absolute top-[-12px] right-[-12px] px-1 bg-[#717fe0] text-center text-[12px] text-white font-bold ">{totalQuantity}</p></Link>

          <Link to="/likePage" onClick={() => setPage("like")} className={page == "like" ? "text-[#717fe0] relative hover:text-[#717fe0]" : "relative hover:text-[#717fe0]"}><IoMdHeartEmpty size={24} /><p className="z-1 absolute top-[-10px] right-[-10px] px-1 bg-[#717fe0]  text-center text-[12px] text-white font-bold ">0</p></Link>
